...
2. **`nodes/AutoElevate/AutoElevate.node.json`** — `"nodeVersion"` field (must match `package.json` version)
...
- Immediately update `AutoElevate.node.json` `nodeVersion` to match
